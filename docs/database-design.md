...
| `ops.outbox_events` | `event_id`, `organization_id`, `event_type`, `aggregate_type`, `aggregate_id`, `payload_version`, `payload_json`, `correlation_id`, `idempotency_key`, `created_at`, `lease_token`, `lease_expires_at`, `attempts`, `available_at`, `delivered_at`, `dead_lettered_at`, `last_error` | durable transactional outbox; unique `(organization_id, event_type, idempotency_key)`; atomic claim with lease via `ops.claim_outbox_event` |
