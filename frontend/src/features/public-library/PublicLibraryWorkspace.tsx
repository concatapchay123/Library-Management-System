...
export function PublicLibraryWorkspace({
  ...
  const handleError = useCallback((err: unknown) => {
    if (err instanceof ProblemDetailsError) {
      if (
        ...
        setUnavailableDetail(err.problem.detail);
        return;
      }
    ...
    if (err && typeof err === 'object') {
      ...
      if (problem) {
        if (
          ...
          setIsEditionUnavailable(true);
          setUnavailableDetail(problem.detail);
          return;
        }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [membersRes, plansRes, subsRes, finesRes, invoicesRes, paymentsRes] = await Promise.all([
        apiClient.publicLibrary.members.list(),
        apiClient.publicLibrary.plans.list(),
        apiClient.publicLibrary.subscriptions.list(),
        apiClient.publicLibrary.fines.list(),
        apiClient.publicLibrary.invoices.list(),
        apiClient.publicLibrary.payments.list(),
      ]);
  ...
  return (
    ...
    >
      ...
      {/* RFC 7807 Problem Details Banner */}
      {problemDetails && (
